...
void readRegisterData()
{
    ...
    // **Instantaneous Tripping**: Immediately trip if current exceeds 26A
    if (stableCurrent > 26.0) {
        Serial.println("----------------------------------------------");
        Serial.println("Instantaneous tripping due to high current (> 26A)!");
        ...
        return; // Skip the inverse time tripping process since we already tripped
    }

    // **Inverse Time Tripping**: Use operating time if current is between PICKUP_CURRENT and 26A
    if (stableCurrent > PICKUP_CURRENT && stableCurrent <= 26.0 && !relayTripped) {
        Serial.println("----------------------------------------------");    
        ...
        stableCurrent = stabilizeCurrentReading(); // Get stable current reading

        // Apply current correction
        if (stableCurrent >= 5.0 && stableCurrent <= 8.0) {
        ...
        // Calculate theoretical operating time
        float operating_time = calculateOperatingTime(stableCurrent);
        Serial.print("Theoretical Operating Time: ");
        Serial.print(operating_time, 2);
        ...
        unsigned long totalDelay = (static_cast<unsigned long>(operating_time * 1000)) - (timeElapsed + 200);
...
float stabilizeCurrentReading() {
    ...
    float threshold = 0.05; // Threshold for the difference between current readings (in Amps)
