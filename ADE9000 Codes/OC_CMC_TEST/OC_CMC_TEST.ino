...
void readRegisterData()
{
    ...
    if (stableCurrent > PICKUP_CURRENT && !relayTripped) {
        ...
        float correct_stableCurrent = applyCurrentCorrectionC(stableCurrent);  // Call the correction function

        // Print the final stabilized current
        Serial.print("Final Stabilized Current: ");
        Serial.print(correct_stableCurrent, 2);
        Serial.println(" A");

        // Calculate theoretical operating time
        float operating_time = calculateOperatingTime(correct_stableCurrent);
